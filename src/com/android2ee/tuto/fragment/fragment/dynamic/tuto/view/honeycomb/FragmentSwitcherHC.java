 ...
 * <li>com.android2ee.tuto.fragment.fragment.dynamic.tuto.view.honeycomb</li>
 * <li>19 oct. 2012</li>
 * 
 * <li>======================================================</li>
 *
 ...
 * *****************************************************************************************************************</br>
 */
package com.android2ee.tuto.fragment.fragment.dynamic.tuto.view.honeycomb;
...
import com.android2ee.tuto.fragment.fragment.dynamic.tuto.view.honeycomb.detail.DetailFragmentHC;
import com.android2ee.tuto.fragment.fragment.dynamic.tuto.view.honeycomb.gen.MActivity;
import com.android2ee.tuto.fragment.fragment.dynamic.tuto.view.honeycomb.main.MainFragmentHC;
 ...
 */
@SuppressLint("NewApi")
public class FragmentSwitcherHC implements MainFragmentCallBack {
	private final String tag = "FragmentSwitcherHC";
	/**
	 * The main activity
	 */
	MActivity activity;
	 ...
	 * NEVER DO THAT
	 */
	// DetailFragmentHC detailFragmentFirstPane, detailFragmentSecondPane;
	/**
	 ...
	 * 
	 */
	public FragmentSwitcherHC(MActivity parent) {
		super();
		activity = parent;
	...
	public void initialize(boolean creationMode) {
		...
		if (twoPanes) {
			...
			// destroy the second fragment recreation if it exists
			secondFragmentRelease();
			// hide the second layout
	...
	private void firstFragmentInitialization() {
		...
		if (creationMode != (mainFragment == null)) {
			log(Log.ERROR, tag, "twoFragmentsInitialization : creationMode!=(mainFragment==null), [creationMode = "
					+ creationMode + " && (mainFragment==null) = " + (mainFragment == null));
		}
		if (null == mainFragment) {
			...
			log(Log.VERBOSE, tag,
					"twoFragmentsInitialization : fTransaction.add(R.id.firstpane, mainFragment, mainFragmentTag);");
		} else {
			...
			log(Log.VERBOSE, tag,
					"onResumeOnePane : fTransaction.replace(R.id.firstpane, mainFragment, mainFragmentTag);");
		}
	...
	private void secondFragmentsInitialization() {
		...
		DetailFragmentHC detailFragmentSecondPane;
		detailFragmentSecondPane = (DetailFragmentHC) fm.findFragmentByTag(detailFragmentSPTag);
		...
		if (null == detailFragmentSecondPane) {
			...
			detailFragmentSecondPane = new DetailFragmentHC();
			// add the information on the selected item using the arguments pattern
			Bundle bundle = new Bundle();
			bundle.putInt(DetailFragmentHC.ITEM_ID, selectedItem);
	...
	/**
	 * Remove the 2 panes fragment because it is not needed
	 */
	private void secondFragmentRelease() {
		// now depending on the configuration of the fragment instantiate the right one
		FragmentManager fm = activity.getFragmentManager();
		FragmentTransaction fTransaction = fm.beginTransaction();
		// retrieve the main fragment
		...
		if (null != detailFragmentSecondPane) {
			...
			fTransaction.remove(detailFragmentSecondPane);
		}
	...
	public void onItemSelectedOnePane(int itemId) {
		...
		// (DetailFragmentHC) fm.findFragmentByTag(detailFragmentFPTag) should be null
		...
		FragmentManager fm = activity.getFragmentManager();
		DetailFragmentHC detailFragmentFirstPane = (DetailFragmentHC) fm.findFragmentByTag(detailFragmentFPTag);
	...
	public void onItemSelectedTwoPanes(int itemId) {
		...
		DetailFragmentHC detailFragmentSecondPane = (DetailFragmentHC) fm.findFragmentByTag(detailFragmentSPTag);
		if (null == detailFragmentSecondPane) {
	...
	public void fragmentLog(int level, String tag) {
		if (MApplication.DEBUG) {
			...
			DetailFragmentHC dfSP = (DetailFragmentHC) fm.findFragmentByTag(detailFragmentSPTag);
			DetailFragmentHC dfFP = (DetailFragmentHC) fm.findFragmentByTag(detailFragmentFPTag);
			MainFragmentHC mf = (MainFragmentHC) fm.findFragmentByTag(mainFragmentTag);
